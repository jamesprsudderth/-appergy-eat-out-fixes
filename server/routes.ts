...
A user is dining at "${restaurantName}" and needs safe dish suggestions.

**User's allergies:** ${allergies || "None specified"}
**User's dietary preferences:** ${preferences || "None specified"}

Suggest 5 dishes that would typically be safe for this user at this type of restaurant.
For each dish, provide the dish name followed by a dash and a brief description explaining why it's likely safe.

IMPORTANT:
- Consider the restaurant's likely cuisine based on its name
- Avoid any dishes containing the user's allergens
- Respect dietary preferences (vegan, vegetarian, kosher, halal, etc.)
- Note any common modifications the user should request

Return ONLY a JSON array of 5 strings, each in the format "Dish Name - Brief safety description".
Example: ["Grilled Salmon - No common allergens, naturally gluten-free", ...]`;
